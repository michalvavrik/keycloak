...
/**
 * Legacy (non-Quarkus) JPA connection provider used only by the embedded Undertow test servers
 * ({@code org.keycloak.testsuite.KeycloakServer} in this module and the {@code auth-server-undertow}
 * Arquillian container, which depends on this module). The Quarkus distribution uses
 * {@code QuarkusJpaConnectionProviderFactory} instead, so this factory (together with
 * {@code META-INF/services/org.keycloak.connections.jpa.JpaConnectionProviderFactory} and
 * {@code default-persistence.xml}) is deliberately kept out of the runtime distribution.
 * <p>
 * An identical copy lives in {@code testsuite/model} test sources for the model test suite, which does
