...
class KcAdmV2CommandBuilder {
    ...
    static void addCommands(CommandLine cli, KcAdmV2CommandDescriptor descriptor) {
        for (ResourceDescriptor resource : descriptor.getResources()) {
            ...
            CommandDescriptor patchDescriptor = null;
            ...
            for (CommandDescriptor cmd : resource.getCommands()) {
                ...
                if (KcAdmV2DescriptorBuilder.CMD_NAME_GET.equals(cmd.getName())) {
                    getDescriptor = cmd;
                } else if (KcAdmV2DescriptorBuilder.CMD_NAME_PATCH.equals(cmd.getName())) {
                    patchDescriptor = cmd;
                }
            }

            if (getDescriptor != null && patchDescriptor != null) {
                groupCli.addSubcommand(CMD_EDIT, buildEditCommand(getDescriptor, patchDescriptor));
            }
    ...
    private static CommandLine buildEditCommand(CommandDescriptor getCmd, CommandDescriptor patchCmd) {
        String resourceName = getCmd.getResourceName();

        KcAdmV2EditCmd executor = new KcAdmV2EditCmd(getCmd, patchCmd);
        CommandSpec spec = CommandSpec.forAnnotatedObject(executor);
        spec.name(CMD_EDIT);
