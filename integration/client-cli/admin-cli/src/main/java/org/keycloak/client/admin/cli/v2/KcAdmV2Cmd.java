...
public class KcAdmV2Cmd extends AbstractTargetAuthOptionsCmd {
    ...
    protected void configureCommandLine(CommandLine cli) {
        ...
        if (help != null) {
            // do not show help under the "Connection options" in the root command help;
            // users don't need to use "--help" for the root command and our root command help already tells users
            // to use the "--help" for sub-commands
    ...
    // point here is that the synopsis generated by Picocli is nicer than custom synopsis, hence we use it where it
    // makes a difference like the client sub-commands while showing the [CONNECTION OPTIONS] hint, here we don't
    // want to show them, so we need to build a custom synopsis for the 2 config sub-commands that are easy to understand
    private static void setConfigSubcommandSynopsis(CommandLine subCli) {
        ...
        for (OptionSpec opt : spec.options()) {
            if (!opt.hidden()) {
                synopsis.append(" [").append(opt.longestName()).append("=").append(opt.paramLabel()).append("]");
            }
        }
