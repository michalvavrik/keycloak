...
public class KcAdmCliV2Test {
    ...
    @Test
    public void testConfigOptionAcceptedAtSubcommandLevel() {
        CommandLine cli = createCli();
        StringWriter sw = new StringWriter();
        cli.setOut(new PrintWriter(sw));

        int exitCode = cli.execute("client", "list", "--config", "/tmp/some.config");
        assertEquals("Exit code should be 0", 0, exitCode);
    }

    @Test
    public void testConfigOptionAcceptedAtGroupLevel() {
        CommandLine cli = createCli();
        StringWriter sw = new StringWriter();
        cli.setOut(new PrintWriter(sw));
        cli.setErr(new PrintWriter(new StringWriter()));

        int exitCode = cli.execute("client", "--config", "/tmp/some.config", "list");
        assertEquals("Exit code should be 0", 0, exitCode);
    }
