...
If your keystore does not have one of these extensions, you must set the `https-key-store-type` option. For BCFKS keystores (used in FIPS mode), the type is set automatically when `--fips-mode=strict` is enabled. Similarly, `.truststore` is recognized for trust stores.
