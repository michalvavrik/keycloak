...
https://docs.redhat.com/en/documentation/red_hat_build_of_openjdk/21/html/configuring_red_hat_build_of_openjdk_21_on_rhel_with_fips/openjdk-default-fips-configuration#security_providers[FIPS enabled security providers].
...
To have Kerberos working, make sure that a `SunJGSS` security provider is available. In FIPS enabled RHEL 9 in OpenJDK 21 or later, the `XMLDSig` security provider may be already enabled
in the `java.security` by default and the same applies with recent OpenJDK 17 versions. But with older OpenJDK 17, it may not be enabled by default, which means that SAML effectively cannot work.
...
Adding this security provider should work well. In fact, it is FIPS compliant and is already included by default in recent versions of OpenJDK 17, and in OpenJDK 21 or later.
...
* Security file `kc.java.security` with added provider for SAML (Not needed with OpenJDK 21 or later, and recent OpenJDK 17 versions)
