...
and of the verifiable credential offer emails of the OID4VCI feature.
...
The `multi-site` feature, used for multi-cluster v1 deployments with an external {jdgserver_name} cluster, is deprecated and will be removed in a future major release. The `stateless` feature (multi-cluster v2) is the recommended replacement. It stores session data in the database, eliminating the need for an external {jdgserver_name} cluster, its cross-site replication, and the associated fencing automation.

If you are using `--features=multi-site`, migrate to `--features=stateless` before upgrading to a future release. For detailed migration instructions, see https://www.keycloak.org/high-availability/multi-cluster-v2/migrate-from-v1-to-v2[Migrating from multi-cluster v1 to v2] in the {highavailabilityguide_name}.
...
When *Full Scope Allowed* is enabled (which is the default for new clients), every access token issued for that client contains _all_ roles the authenticated user holds across all clients and the realm. This is convenient during early development, but is a security risk in production: a single compromised token grants access to every resource the user can reach. The correct approach is to disable *Full Scope Allowed* and explicitly configure link:{adminguide_link}#_role_scope_mappings[role scope mappings] so that each client's tokens contain only the roles that client actually needs.

==== Deprecation warning in the server log

Starting with this release, {project_name} logs a `WARN`-level message at token-issuance time for every client that still has *Full Scope Allowed* enabled. The warning is suppressed for the built-in admin clients `security-admin-console` and `admin-cli` (see below).

To silence the warning for all other clients temporarily, disable logging for the category `org.keycloak.protocol.oidc.endpoints.TokenEndpoint.full-scope-allowed`. However, the recommended action is to keep the warning enabled and instead migrate the clients that produce it, so that your deployment is prepared for the full removal of the switch in a future release.

==== Built-in admin clients

The built-in clients `security-admin-console` and `admin-cli` currently have *Full Scope Allowed* enabled and the deprecation warning is intentionally suppressed for them. The {project_name} team will fix these clients in an upcoming release as part of the planned migration path.

It is not strictly needed to manually disable *Full Scope Allowed* for these clients in your own environment. Disabling it manually can break Admin Console login immediately, and may require additional manual migration steps in future {project_name} upgrades.
If you prefer to disable it for the built-in clients, you may need some further actions (EG. Create client scope with the appropriate role scopes and add that client scope to the built-in admin clients).
...
To enforce that all newly created or updated clients in a realm have *Full Scope Allowed* disabled automatically, configure a client policy with the `full-scope-disabled` executor. See link:{adminguide_link}#_full_scope_disabled_policy[Enforcing Full Scope Allowed to be disabled] in the {adminguide_name} for step-by-step instructions, including how to exclude built-in clients from the policy.
