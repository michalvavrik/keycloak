...
public class KcAdmV2ClientCLITest {
    ...
    @Test
    void testInvalidFieldsSpecShowsError() {
        // Trailing comma in --fields is invalid — should fail with a clear error, not silently return unfiltered JSON
        CommandResult result = kcAdmV2Cmd("client", "list", "-c", "-F", "clientId,");

        assertThat("invalid fields spec should fail", result.exitCode(), is(not(0)));
        assertThat("should tell user about the invalid fields specification",
                result.err(), containsString("Invalid fields specification"));
    }
