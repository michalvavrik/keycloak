...
public class KcAdmV2ClientCLITest {
    ...
    void testRealmOverrideWithMasterConfig() {
        // Config login is for master — targeting another realm should work with info message
        ...
        assertThat("should print info about cross-realm targeting",
                result.err(), containsString("target realm '" + realm.getName() + "'"));
    ...
    @Test
    void testRealmOverrideFromNonMasterConfigFails() throws Exception {
        // checks validation failure - if someone authenticates with realm A
        // and then specify realm B using '-r B', this should fail as we validate that credentials
        // were acquired for a different realm that is not the master realm

        // Create a config that looks like it was authenticated against a non-master realm
        String nonMasterConfig = new File(tempDir, "non-master.config").getAbsolutePath();
        // Login to master first (to get valid credentials)
        ...
        cli.execute("config", "credentials",
                "--server", keycloakUrls.getBase(),
                "--realm", "master",
                "--client", Config.getAdminClientId(),
                "--secret", Config.getAdminClientSecret(),
                "--config", nonMasterConfig);

        // Modify the config to pretend it's for a non-master realm
        String configContent = Files.readString(new File(nonMasterConfig).toPath());
        configContent = configContent.replace("\"realm\" : \"master\"", "\"realm\" : \"my-realm\"");
        Files.writeString(new File(nonMasterConfig).toPath(), configContent);

        // Now try to target a different realm — should fail
        CommandResult result = executeWithConfig(nonMasterConfig,
                "client", "list", "-c", "-r", "some-other-realm");

        assertThat("should fail when targeting different realm from non-master config",
                result.exitCode(), is(not(0)));
        assertThat(result.err(), containsString("Cannot use --realm"));
        assertThat(result.err(), containsString("my-realm"));
    }
    ...
    @Test
    void testClientWithoutSecretPromptsInNonInteractive() {
        // --client without --secret and no --user should trigger secret prompting
        CommandResult result = kcAdmV2CmdNoConfig("client", "list", "-c",
                "--server", keycloakUrls.getBase(),
                "--client", Config.getAdminClientId());

        assertThat("should fail when secret can't be prompted",
                result.exitCode(), is(not(0)));
        assertThat(result.err(), containsString("Console is not active"));
    }

    @Test
    void testTlsOptionsIgnoredForHttpWithWarning() {
        CommandResult result = kcAdmV2Cmd("client", "list", "-c",
                "--truststore", "/nonexistent/truststore.jks");

        assertThat("should succeed (truststore ignored for HTTP): " + result.err(),
                result.exitCode(), is(0));
        assertThat(result.err(), containsString("TLS options are ignored"));
    }
    ...
    void testServerAndRealmWithoutCredentialsFails() {
        // --server and --realm provided but no credentials — should not complain about missing realm
        CommandResult result = kcAdmV2CmdNoConfig("client", "create", "saml",
                "--server", keycloakUrls.getBase(), "--realm", "master");

        assertThat("should fail without credentials", result.exitCode(), is(not(0)));
        assertThat("should not complain about missing realm",
                result.err(), not(containsString("No realm")));
        assertThat(result.err(), containsString("No credentials configured"));
    }
    ...
    private CommandResult executeWithConfig(String config, String... args) {
        CommandLine cli = Globals.createCommandLine(new KcAdmV2Cmd(), KcAdmMain.CMD, new PrintWriter(System.err, true));
