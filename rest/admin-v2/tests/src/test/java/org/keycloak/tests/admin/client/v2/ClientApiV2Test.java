...
public class ClientApiV2Test extends AbstractClientApiV2Test{
    ...
    @Test
    public void clientSecretCanBeRegenerated() throws IOException {
        String clientId = "client-secret-generation";

        // anonymous user => 401
        HttpPost generateRequest = new HttpPost(getClientsApiUrl() + "/" + clientId + "/generate-secret");
        try (var response = client.execute(generateRequest)) {
            assertEquals(401, response.getStatusLine().getStatusCode());
        }

        // insufficient permissions => 403
        setAuthHeader(generateRequest, noAccessAdminClient);
        try (var response = client.execute(generateRequest)) {
            assertEquals(403, response.getStatusLine().getStatusCode());
        }

        // client does not exist => 404
        setAuthHeader(generateRequest);
        try (var response = client.execute(generateRequest)) {
            assertEquals(404, response.getStatusLine().getStatusCode());
        }
        ...
        createRequest.setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        OIDCClientRepresentation.Auth createdAuth = getResultingAuthConfig(null, createRequest, clientId);
        assertThat(createdAuth, nullValue());

        // client exists, but it is a public client => 400
        try (var response = client.execute(generateRequest)) {
            assertEquals(400, response.getStatusLine().getStatusCode());
        }

        // switch authentication method to jwt
        OIDCClientRepresentation.Auth authWithSecret = new OIDCClientRepresentation.Auth();
        authWithSecret.setMethod(JWTClientAuthenticator.PROVIDER_ID);
        authWithSecret.setSecret("m-n-o-p-q-r-9-8-7-6-5");
        HttpEntityEnclosingRequestBase request = new HttpPut(getClientsApiUrl() + "/" + clientId);
        request.setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        OIDCClientRepresentation.Auth putAuth = getResultingAuthConfig(authWithSecret, request, clientId);
        ...
        // client exists, but it uses other method than the client secret => 400
        try (var response = client.execute(generateRequest)) {
            assertEquals(400, response.getStatusLine().getStatusCode());
        }

        // switch authentication method to client secret
        authWithSecret.setMethod(ClientIdAndSecretAuthenticator.PROVIDER_ID);
        authWithSecret.setSecret("s-t-u-v-w-9-8-7-6-5");
        ...
        // client exists and is using the client secret => regenerate
        try (var response = client.execute(generateRequest)) {
            assertEquals(200, response.getStatusLine().getStatusCode());
            newSecret = EntityUtils.toString(response.getEntity(), StandardCharsets.UTF_8);
            assertThat(newSecret, not(emptyOrNullString()));
            assertThat(newSecret, not(is(authWithSecret.getSecret())));
        }

        // the generated secret should be stored on the client, verify it using the GET client operation
        HttpGet getRequest = new HttpGet(getClientsApiUrl() + "/" + clientId);
        setAuthHeader(getRequest);
        try (var response = client.execute(getRequest)) {
            assertEquals(200, response.getStatusLine().getStatusCode());
            OIDCClientRepresentation client = mapper.createParser(response.getEntity().getContent()).readValueAs(OIDCClientRepresentation.class);
            assertEquals(clientId, client.getClientId());
            assertThat(client.getAuth().getSecret(), is(newSecret));
        }
    }
    ...
    public void clientSecretGenerationInPutRequest() throws IOException {
        ...
        // in which case we expect to turn the client into public one
