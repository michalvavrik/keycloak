...
public class ClientApiV2Test {
    ...
    @Test
    public void secretIsGeneratedForClientSecretAuthMethod() throws IOException {
        HttpPost request = new HttpPost(HOSTNAME_LOCAL_ADMIN);
        setAuthHeader(request);
        request.setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        ...
        OIDCClientRepresentation.Auth createdAuth = getResultingAuthConfig(auth, request, "secret-generation-post");
    ...
    @Test
    public void clientSecretGeneratedForPatchRequest() throws IOException {
        String clientId = "secret-generation-patch";

        // create public client
        HttpEntityEnclosingRequestBase request = new HttpPost(HOSTNAME_LOCAL_ADMIN);
        setAuthHeader(request);
        request.setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
        ...
        request = new HttpPatch(HOSTNAME_LOCAL_ADMIN + "/" + clientId);
        setAuthHeader(request);
        request.setHeader(HttpHeaders.CONTENT_TYPE, AdminApi.CONTENT_TYPE_MERGE_PATCH);
        ...
        // if we don't specify any auth, we still should keep the secret - that is behavior of the API v1 as well;
        // however, it also means that using patch to turn client into public is not possible
        patchedAuth = getResultingAuthConfig(null, request, clientId);
        assertThat(patchedAuth, notNullValue());
        assertThat(patchedAuth.getSecret(), is(newlyGeneratedSecret));

        // if we specify client-secret auth without secret, we should keep the secret instead of regenerating the secret
        // because there is a dedicated endpoint used to re-generated the secret
    ...
    @Test
    public void clientSecretIsKeptIfMissingInUpdateEntity() {
