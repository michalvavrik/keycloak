...
public class KcAdmV2EditCLITest extends AbstractKcAdmV2CLITest {
    ...
    @Test
    void testEditAppliesPatch() throws Exception {
        CommandResult createResult = createClient("edit-applies-patch", "--enabled", "true");
        ...
        CommandResult result = kcAdmV2Cmd("client", "edit", "edit-applies-patch");

        assertThat("edit should succeed: " + result.err(), result.exitCode(), is(0));
        assertThat("should show changes to apply", result.err(), containsString("Changes to apply"));
        assertThat("diff should show the enabled change and patched resource should reflect it",
                result.out(), containsString("\"enabled\" : false"));
        assertThat("patched resource should contain the clientId",
                result.out(), containsString("\"clientId\" : \"edit-applies-patch\""));

        CommandResult getResult = kcAdmV2Cmd("client", "get", "edit-applies-patch");
