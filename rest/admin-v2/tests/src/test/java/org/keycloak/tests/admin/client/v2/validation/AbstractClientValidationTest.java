...
public abstract class AbstractClientValidationTest extends AbstractClientApiV2Test {
    ...
    public void testAuthMethodInvalidProvider() throws Exception {
        ...
        try (var response = client.execute(request)) {
            ...
            assertThat(body.violations(), hasItem(containsString("not a valid client authenticator")));
        }
    }
    ...
    @Test
    public void testSignatureAlgorithmMaxLength() throws Exception {
        var request = getRequest(false);
        setAuthHeader(request);

        String longAlg = "RSA_SHA" + "X".repeat(44);
        request.setEntity(new StringEntity("""
                {
                    "protocol": "saml",
                    "clientId": "%s",
                ...
                """.formatted(getPayloadClientId(false), longAlg)));

        try (var response = client.execute(request)) {
            assertThat(response.getStatusLine().getStatusCode(), is(400));
            var body = mapper.createParser(response.getEntity().getContent()).readValueAs(ViolationExceptionResponse.class);
            assertThat(body.error(), is("Provided data is invalid"));
            assertThat(body.violations(), hasItem("signatureAlgorithm: size must be between 0 and 50"));
        }
    }
    ...
    public void testSignatureAlgorithmInvalidValue() throws Exception {
        ...
        try (var response = client.execute(request)) {
            ...
            var body = mapper.createParser(response.getEntity().getContent()).readValueAs(ViolationExceptionResponse.class);
            assertThat(body.error(), is("Provided data is invalid"));
            assertThat(body.violations(), hasItem(containsString("signatureAlgorithm")));
            assertThat(body.violations(), hasItem(containsString("must be a valid SAML signature algorithm")));
        }
    }
